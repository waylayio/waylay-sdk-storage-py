...
class ListQuery(WaylayBaseModel):
    ...
    fetch_content_type: Annotated[
        StrictBool | None,
        Field(
            description="If true, fetch content type for each object when not already available from the listing (e.g. on s3 stores). This is less efficient but may be necessary for certain use cases."
        ),
    ] = None
